...
test.describe('misc', () => {
  test('set the display name', async ({ poms: { settings, displayName } }) => {
    await settings.goto();
  ...
  test('change password', async ({
    poms: { settings, changePassword, login },
    credentials,
  }) => {
  ...
  test('add and remove totp', async ({ poms: { settings, totp } }) => {
    await settings.goto();
  ...
  multiTest('disconnect RP', async ({ credentials, browsers: [a, b] }) => {
    ...
    expect(services.length).toEqual(3); //TODO this should be 2 but there's a mysterious empty one
